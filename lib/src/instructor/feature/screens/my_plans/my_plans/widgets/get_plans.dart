...
class MyPlansUI extends StatelessWidget {
  final EdgeInsetsGeometry? margin;
  final EdgeInsetsGeometry? padding;

  const MyPlansUI({super.key, this.margin, this.padding});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<GetPlansCubit, GetPlansState>(
      builder: (context, state) {
        if (state.status == GetPlansStatus.loading) {
          return Center(child: AppLoading());
        } else if (state.status == GetPlansStatus.success) {
          return RefreshIndicator(
            onRefresh: () async {
              context.read<GetPlansCubit>().getPlans();
            },
                            ...
                            return ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final correspondingSub =
                                  ...
                                  (sub) =>
                                      sub.mealPlan!.planName == _plan.planName,
                                  orElse: () => null,
                                );
        ...
        } else if (state.status == GetPlansStatus.error) {
          return const Center(
            child: ErrorTexts(
