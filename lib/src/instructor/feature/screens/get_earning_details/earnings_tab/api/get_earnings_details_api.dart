...
class GetEarningDetailsApi {
  static final Dio _dio = Dio(BaseOptions(baseUrl: ApiUrl.kApiBaseUrl));

  static Future<List<EarningDetailsModel>> getEarningDetails(
      {CancelToken? cancelToken}) async {
    final String endpoint = ApiUrl.kGetPayout;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        List<EarningDetailsModel> data = (response.data as List)
            .map((e) => EarningDetailsModel.fromJson(e))
            .toList();

        return data;
      } else if (response.statusCode == 401) {
        await RefreshTokenService.refreshAccessToken();
        return await getEarningDetails(cancelToken: cancelToken);
      } else {
