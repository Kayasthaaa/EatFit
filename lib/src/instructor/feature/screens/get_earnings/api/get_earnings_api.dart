...
class GetEarningsApi {
  static final Dio _dio = Dio(BaseOptions(baseUrl: ApiUrl.kApiBaseUrl));

  static Future<EarningsModels> getEarnings({CancelToken? cancelToken}) async {
    String endpoint = ApiUrl.kGetEarnings;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        EarningsModels data = EarningsModels.fromJson(response.data);
        return data;
      } else if (response.statusCode == 401) {
        await RefreshTokenService.refreshAccessToken();
        return await getEarnings(cancelToken: cancelToken);
      } else {
